...
public class BrokerModelImpl implements BrokerModel {
    ...
    public void addCustomer(Customer cust)
        ...
        try {
            // primero buscamos si el Customer existe en la lista, por su ID
            getCustomer(cust.getId());
            // si no existe, se devuleve una BrokerException...
            throw new BrokerException("Duplicate Id : " + cust.getId()); // Lanzamos una excepción que se salta el catch
        } catch (BrokerException be) { // ... se recoge la excepcion de getCustomer()
            // ... y se añade el nuevo custome a la lista de customers
            customers.add(cust);
        }
    }
    ...
    public Customer getCustomer(String id)
            throws BrokerException { //Significa que propaga la Excepción del tipo BrokerException al llamante
        Customer cr = null;
        boolean custFound = false;
        ...
        // si encuentra el Customer-ID lo devuelve
        if (custFound) {
            return cr;
            // pero si no, lanza una excepción del tipo BrokerException con un String
        } else {
